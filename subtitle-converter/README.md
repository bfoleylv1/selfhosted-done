...
Then open <http://localhost:20309>.

Swarm:

```bash
docker stack deploy -c swarm/docker-stack.yml subtitle-converter
```
...
```bash
docker inspect --format '{{.State.Health.Status}}' subtitle-converter
```
